  ...
  for (let i = 0; i < searchText.length; i++) {
    ...
    while (j > 0 && searchText[i] !== searchPattern[j]) {
      detailedSteps.push({
        ...
        type: "mismatch",
        shift: j - failure[j - 1],
        description: `Mismatch at text[${i}]='${
          searchText[i]
        }' vs pattern[${j}]='${searchPattern[j]}'. Shift pattern by ${
          j - failure[j - 1]
        } positions.`,
      });
      j = failure[j - 1];
...
const rabinKarpSearchDetailed = (text, pattern, caseInsensitive = false) => {
  ...
  for (let i = 0; i <= textLength - patternLength; i++) {
    ...
    if (i < textLength - patternLength) {
      textHash =
        (base * (textHash - searchText.charCodeAt(i) * h) +
          searchText.charCodeAt(i + patternLength)) %
        prime;
  ...
  for (let i = 1; i < combined.length; i++) {
    if (i <= r) {
      ...
      detailedSteps.push({
        ...
        type: "z-box",
        zValue: z[i],
        description: `Using Z-box: Z[${i}] = min(${r - i + 1}, Z[${i - l}]) = ${
          z[i]
        }`,
      });
    }
...
export default function StringMatcher() {
  ...
  const totalTime = chartData.reduce(
    (sum, d) => sum + (Number(d.time) || 0),
    0
  );
  const totalComparisons = chartData.reduce(
    (sum, d) => sum + (Number(d.comparisons) || 0),
    0
  );

  const handlePlayToggle = () => {
    const steps = results?.pattern_0?.detailedSteps || [];
    if (!isPlaying) {
  ...
  }, [isPlaying, currentStep, playSpeed, results?.pattern_0?.detailedSteps]);
  ...
  const runAlgorithm = () => {
    ...
    patternList.forEach((pattern, index) => {
      ...
      const result = algorithms[selectedAlgorithm].func(
        text,
        pattern,
        caseInsensitive
      );
    ...
    const totalMatches = Object.values(allResults).reduce(
  ...
  const renderHighlightedText = (
    ...
    const classAt = new Array(length).fill(null);

    // Fill match ranges
    Object.values(allResults).forEach((result, patternIndex) => {
      if (result.matches && Array.isArray(result.matches) && result.pattern) {
        const color = `${
          colors[patternIndex % colors.length]
        } text-white rounded px-1 font-bold transition-smooth`;
        result.matches.forEach((start) => {
          const end = Math.min(length, start + result.pattern.length);
          for (let i = Math.max(0, start); i < end; i++) classAt[i] = color;
        });
      }
    });

    // Overlay active window during playback
    if (
      showActive &&
      allResults?.pattern_0?.detailedSteps &&
      currentStep >= 0
    ) {
      const step = allResults.pattern_0.detailedSteps[currentStep];
      const activePattern = allResults.pattern_0.pattern;
      if (step && typeof step.position === "number" && activePattern) {
        const start = Math.max(0, step.position);
        const end = Math.min(length, start + activePattern.length);
        for (let i = start; i < end; i++)
          classAt[
            i
          ] = `bg-yellow-500 text-white rounded px-1 font-bold transition-smooth`;
      }
    }
    ...
    let i = 0;
    while (i < length) {
      const currentClass = classAt[i];
      let j = i + 1;
      while (j < length && classAt[j] === currentClass) j++;
      const chunk = textStr.slice(i, j);
      if (currentClass) {
        segments.push(
          <span key={`${i}-${j}`} className={currentClass}>
            {chunk}
          </span>
        );
      } else {
        segments.push(<span key={`${i}-${j}`}>{chunk}</span>);
      }
      i = j;
    }
        ...
        <div className="w-full">
          ...
          <div className="text-center space-y-4 mb-8">
            ...
            <p className="text-muted-foreground text-lg">
              Interactive visualization of KMP, Rabin-Karp, and Z-Algorithm with
              real-time analysis
            </p>
          </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setCurrentStep(
                              Math.min(
                                results.pattern_0.detailedSteps.length - 1,
                                currentStep + 1
                              )
                            )
                          }
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-sm">
                        <span>
                          ...
                          {results.pattern_0.detailedSteps.length}
                        </span>
                        <div className="flex items-center gap-2">
                        ...
                        result.pattern && (
                          <div key={index} className="flex items-center gap-2">
                            <Badge variant="outline">
                              "{result.pattern}": {result.matches?.length || 0}{" "}
                              matches
                            </Badge>
                            {result.time && (
